...
fc.exe "%OUTPUT_DIR%\empty.txt" "%TEST_DIR%\test-empty.txt"
if ERRORLEVEL 1 goto err

rem Тест 2 – одна строка
%PROGRAM% "%TEST_DIR%\test-hello.txt" "%OUTPUT_DIR%\hello.txt"
if ERRORLEVEL 1 goto err
fc.exe "%OUTPUT_DIR%\hello.txt" "%TEST_DIR%\test-hello.txt"
if ERRORLEVEL 1 goto err

rem Тест 3 – много строк
%PROGRAM% "%TEST_DIR%\test-poem.txt" "%OUTPUT_DIR%\poem.txt"
if ERRORLEVEL 1 goto err
fc.exe "%OUTPUT_DIR%\poem.txt" "%TEST_DIR%\test-poem.txt"
if ERRORLEVEL 1 goto err

rem Тест 4 – числа
%PROGRAM% "%TEST_DIR%\test-numbers.txt" "%OUTPUT_DIR%\numbers.txt"
if ERRORLEVEL 1 goto err
fc.exe "%OUTPUT_DIR%\numbers.txt" "%TEST_DIR%\test-numbers.txt"
if ERRORLEVEL 1 goto err

rem Тест 5 – отсутствующий файл
%PROGRAM% "%TEST_DIR%\missing.txt" "%OUTPUT_DIR%\missing.txt" > "%OUTPUT_DIR%\error.txt"
